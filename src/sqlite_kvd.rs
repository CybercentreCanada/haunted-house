...
//         let con = self.connection.get().await?;

//         let query = format!("create table if not exists {name} (
//             key BLOB PRIMARY KEY,
//             value BLOB NOT NULL
//         )");

//         let x = match con.interact(move |con|{
//             let mut statement = con.prepare(&query)?;
//             anyhow::Ok(statement.execute([])?)            
//         }).await {
//             Ok(res) => res?,
