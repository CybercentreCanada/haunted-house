...
impl Database {
    pub async fn update_file_access(&self, hash: &[u8], access: &AccessControl, index_group: &String) -> Result<bool> {
        match self {
            Database::Rocks(local) => local.update_file_access(hash, access, index_group).await,
            // Database::LMDB(local) => local.update_file_access(hash, access, index_group).await,
        }
    }
    ...
    pub async fn select_index_to_grow(&self, index_group: &String) -> Result<(bool, String)> {
        match self {
            ...
            // Database::LMDB(local) => local.select_index_to_grow(index_group).await,
        }
    }
    ...
    pub async fn create_index_data(&self, index_group: &String, index_id: String, meta: Vec<(Vec<u8>, AccessControl)>, new_size: usize) -> Result<()> {
        match self {
            Database::Rocks(local) => local.create_index_data(index_group, index_id, meta, new_size).await
        }
    }
    ...
    pub async fn update_index_data(&self, index_group: &String, index_id: String, meta: Vec<(Vec<u8>, AccessControl)>, index_offset: usize, new_size: usize) -> Result<()> {
        match self {
            Database::Rocks(local) => local.update_index_data(index_group, index_id, meta, index_offset, new_size).await
        }
    }
}
...
impl RocksInterface {
    pub fn new(index_soft_max: usize) -> Result<Database> {
        Ok(Database::Rocks(Self {
            db: todo!(),
            index_soft_max,
        }))
    }
    ...
    pub async fn update_file_access(&self, hash: &[u8], access: &AccessControl, new_index_group: &String) -> Result<bool> {       
